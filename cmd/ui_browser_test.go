...
func TestUIBrowserWorkflows(t *testing.T) {
	...
	nav("/app/jobs")
	check(`document.querySelectorAll('.js-job-check').length === 50 && document.getElementById('process-selected-jobs').disabled`)
	eval(`document.getElementById('select-all-jobs').click()`)
	check(`document.getElementById('process-selected-jobs').disabled && !document.getElementById('queue-selected-jobs').disabled`)
	eval(`Array.from(document.querySelectorAll('.js-job-check')).slice(25).forEach(x=>x.click())`)
	check(`!document.getElementById('process-selected-jobs').disabled && document.getElementById('select-all-jobs').indeterminate`)
