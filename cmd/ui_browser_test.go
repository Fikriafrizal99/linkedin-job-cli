...
func TestUIBrowserWorkflows(t *testing.T) {
	...
	check(`Array.from(document.querySelectorAll('.js-triage-action')).every(x=>!x.disabled) && document.getElementById('select-all-jobs').indeterminate && document.getElementById('selected-jobs-count').textContent.includes('25')`)
